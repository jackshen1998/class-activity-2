...
student_id = seq(1,100, by=1)
student_score=rnorm(100,75,15)
student_score<-ifelse(student_score >= 100, 100,student_score)
student_score = round(student_score)
 df <- data.frame(student_id,student_score)
 interest = sample(c("sport","music","nature","literature"), 100, replace=TRUE)
 df = data.frame(df,interest)
...
```{r}
hist(df$student_score,breaks = c(0,70,80,90,100))
```
...
sc<-c(0,70,80,90,100)
letters<-c("F","C","B","A")
grade<-table(cut(student_score,sc,letters))
grade_student<-cut(student_score,sc,letters,order_result=TRUE)
df<-data.frame(df,grade_student)
```
...
color<-palette(brewer.pal(4,"Oranges"))
#Use named palette in histogram
hist(df$student_score,breaks = c(0,70,80,90,100),col=color)
```
...
box<-brewer.pal(8,"Paired")
boxplot(df$student_score~df$interest, col = box)
```
...
```{r}
log_in<-sample(1:25,100,replace = TRUE)
df2<-cbind(df,log_in)
```
...
```{r}
plot(df2$log_in,df2$student_score,type = "p",main = "Score",col = df2$interest)
...
```{r}
pairs(iris)
#petallength and petalwidth & sepallength and petallength have correlation
```
